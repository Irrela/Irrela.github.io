...
堆栈包含viable prefixes(可行的前缀)。如果在任何时间点，堆栈包含了$$\alpha$$，那么它就会对一些ter字符串$$\beta$$有$$ S \xLongrightarrow{*} \alpha \beta$$。

如果那个时候剩下的输入是$$\gamma$$，那自然无法保证$$S \xLongrightarrow{*}\alpha \gamma$$。input可能不是一个句子。

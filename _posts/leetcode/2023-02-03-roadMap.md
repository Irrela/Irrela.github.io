...
- [Solution](#solution)
  - [Two Pointers](#two-pointers)
    ...
    - [Modified Binary Search](#modified-binary-search)
      - [在有序数组中查找一个特定的值](#在有序数组中查找一个特定的值)
      - [`第一个等于`给定值的元素](#第一个等于给定值的元素)
      - [`第一个严格大于`给定值](#第一个严格大于给定值)
      - [`最后一个等于`给定值的](#最后一个等于给定值的)
      - [`最后一个严格小于`给定值的](#最后一个严格小于给定值的)
      - [`最后一个小于等于`给定值的元素](#最后一个小于等于给定值的元素)
...
二分法存在多种形式以对应不同场景，边界条件十分关键

#### 在有序数组中查找一个特定的值
问题描述： 在有序数组中查找一个特定的值。
边界条件： 主要是确保左右指针正确移动，防止死循环。通常使用 left <= right 作为循环条件。
```java
while (left <= right) {
...
#### `第一个等于`给定值的元素
问题描述： 在有序数组中找到第一个等于给定值的元素。
边界条件： 在找到目标值时，需要检查它是否是第一个等于给定值的元素。通常使用 left < right 作为循环条件。
```java
int binarySearchFirstEqual(int[] nums, int target) {
...
#### `第一个严格大于`给定值
```java
static int binarySearchFirstStrictlyGreater(int[] nums, int target) {
    int low = 0, high = nums.length - 1;
...
#### `最后一个等于`给定值的
```java
int binarySearchLastEqual(int[] nums, int target) {
    int low = 0, high = nums.length - 1;
...
#### `最后一个严格小于`给定值的
```java
static int binarySearchLastStrictlyLess(int[] nums, int target) {
    int low = 0, high = nums.length - 1;
...
#### `最后一个小于等于`给定值的元素
问题描述： 在有序数组中找到最后一个等于给定值的元素。
边界条件： 在找到目标值时，需要检查它是否是最后一个等于给定值的元素。通常使用 left < right 作为循环条件。
```java
int binarySearchLastLessOrEqual(int[] nums, int target) {
    int low = 0, high = nums.length - 1;

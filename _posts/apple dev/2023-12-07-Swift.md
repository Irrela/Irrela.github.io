...
struct BoardView: View {
    var body: some View {
        Text("Hello, World!".uppercased()) // 将文本转为大写，.lowercased() 和 .capitalized() 分别是转为小写和首字母大写
            .font(.title) // 设置字体大小
            .fontWeight(.semibold) // 设置字体粗细
            .bold() // 将文本加粗
            .underline() // 在文本下方添加下划线
            .strikethrough() // 在文本中间添加删除线
            .foregroundColor(.blue) // 设置文本颜色
            
            .baselineOffset(10.0) // 设置文本的基线偏移，负值表示向上偏移，正值表示向下偏移，用于调整行间距
            .kerning(1.0) // 设置字母间距，单位为pixel
            .multilineTextAlignment(.leading) // 设置多行文本的对齐方式为左对齐

            .frame(width: 200, height: 100, alignment: .center) // 设置文本区域的大小和对齐方式
            .minimumScaleFactor(0.1) // 在需要缩小文本以适应框架时，设置文本的最小缩放因子
    }
}

...
- [3. 字符串，向量和数组](#3-字符串向量和数组)
  - [3.4 迭代器介绍](#34-迭代器介绍)
    - [迭代器运算符](#迭代器运算符)
    - [3.4.2 迭代器运算](#342-迭代器运算)
- [6. 函数](#6-函数)
  - [6.1 函数基础](#61-函数基础)
    - [6.1.1 局部静态对象](#611-局部静态对象)
      - [延迟初始化](#延迟初始化)
      - [缓存数据：](#缓存数据)
      - [资源管理](#资源管理)
      - [计数器](#计数器)
...
- [8. IO lib](#8-io-lib)
  - [iostream](#iostream)
    - [8.1.1 IO对象不能拷贝或赋值](#811-io对象不能拷贝或赋值)
    - [8.1.2 流的条件状态](#812-流的条件状态)
      - [IO之前检查流的状态](#io之前检查流的状态)
      - [清除流对象的错误状态](#清除流对象的错误状态)
    - [8.1.3 管理输出缓冲](#813-管理输出缓冲)
      - [`unibuf` \& `nounibuf`](#unibuf--nounibuf)
      - [tie() 关联输入输出](#tie-关联输入输出)
- [9. 顺序容器](#9-顺序容器)
  - [9.1 概述](#91-概述)
  - [9.2 容器类的操作函数](#92-容器类的操作函数)
    - [9.2.1 迭代器](#921-迭代器)
- [12. 动态内存](#12-动态内存)
  - [12.1 智能指针](#121-智能指针)
    - [12.1.0 `new` \& `delete`](#1210-new--delete)
    - [12.1.1 `shared_ptr`](#1211-shared_ptr)
      - [原理：](#原理)
      - [使用场景：](#使用场景)
      - [示例代码：](#示例代码)
    - [使用`make_shared`函数创建`shared_ptr`](#使用make_shared函数创建shared_ptr)
      - [使用场景：](#使用场景-1)
      - [示例代码:](#示例代码-1)
    - [shared\_ptr的拷贝和赋值](#shared_ptr的拷贝和赋值)
    - [多个对象共享成员数据：StrBlob](#多个对象共享成员数据strblob)
- [13. 拷贝控制](#13-拷贝控制)
  - [13.1 拷贝，赋值与销毁](#131-拷贝赋值与销毁)
    ...
    - [13.1.2 拷贝赋值运算符](#1312-拷贝赋值运算符)
      - [重载运算符 (overloaded operator)](#重载运算符-overloaded-operator)
      - [合成拷贝赋值运算符](#合成拷贝赋值运算符)
    - [13.1.3 析构函数](#1313-析构函数)
      - [在实践中，关于析构函数容易踩的坑包括：](#在实践中关于析构函数容易踩的坑包括)
    - [13.1.4 三/五法则](#1314-三五法则)
    - [13.1.5 使用`=default`](#1315-使用default)
    - [13.1.6 使用`=delete`](#1316-使用delete)
      - [管理拷贝控制的权限](#管理拷贝控制的权限)
  ...
  - [13.3 交换操作](#133-交换操作)
    - [自定义swap 和 std::swap](#自定义swap-和-stdswap)
    - [在赋值运算符中使用swap](#在赋值运算符中使用swap)
  - [13.4 拷贝控制示例](#134-拷贝控制示例)
    - [Message and Folder](#message-and-folder)
    - [Message的成员方法实现](#message的成员方法实现)
    - [Message的公共操作工具函数实现](#message的公共操作工具函数实现)
    - [Message的基于公共函数的拷贝控制成员实现](#message的基于公共函数的拷贝控制成员实现)
...
- [Miscellany](#miscellany)
  - [Keyword](#keyword)
    - [explict](#explict)
      - [什么是隐式的类型转换](#什么是隐式的类型转换)
  - [Vector](#vector)
    - [erase](#erase)
    - [sort](#sort)
    - [getline](#getline)
    - [stoi](#stoi)
    - [为什么STL的pop操作不返回被弹出元素？](#为什么stl的pop操作不返回被弹出元素)
    - [C style](#c-style)
    - [Cpp与Java的泛型讨论](#cpp与java的泛型讨论)
...
每个Message可以出现在多个Folder中，但每条Message只有一个副本 -> 如果一条Message发生改变，则从任何包含它的Folder中浏览时，都会看到改变后的内容。
...
private:
    ...
    void add_to_Folders(const Message&); // 将本Message添加到给定的一组Folder
...
这种类通常可以(并且如果它们确实 可以的话，一般应该)使用`标准库容器`来保存它们的数据.

我们将实现标准库 `vector` 类的一个简化版本。我们所做的 一个简化是`不使用模板`，我们的类只用于string, 它被命名为 `Strvec`。

> vector 类将其元素保存在连续内存中。为了获得可接受的性能，vector预`先分配足够的内存`来保存可能需要的更多元素。
> vector 的每个添加元素的成员函数会`检查`是否有空间容纳更多的元素: 
> 如果有，成员函数会在下一个可用位置构造一个对象。
> 如果没有可用空间，vector 就会重新分配空间: 它获得新的空间，将已有元素移动到新空间中，释放旧空间，并添加新元素

1. 我们将使用一个 `allocator` 来获得原始内存:  由于 allocator 分配的内存是`未构造`的 ， 我们将在需要添加新元素时用 allocator 的 construct 成员在原始内存中创建对象
2. 当我们需要删除一个元素时，我们将使用 destroy 成员来销毁元素。

3. 每个StrVec有三个`指针成员`指向其元素所使用的内存:
   - elements，指向分配的内存中的首元素
   - first_free，指向最后一个实际元素之后的位置 
   - cap ，指向分配的内存末尾之后的位置

4. 除了这些指针成员之外，StrVec 还有一个名为 alloc 的静态成员，其类型为 allocator<string> 。alloc 成员会分配 Strvec 使用的内存。
5. 4 个工具函数:
   - `a1loc_n_copy` 会分配内存，并拷贝一个给定范围中的元素。
   - `free` 会销毁构造的元素并释放内存。
   - `chk_n_alloc` 保证 strVec 至少有容纳一个新元素的空间。如果没有空间添加新元素，chk_n_alloc 会调用 reallocate 来分配更多内存。
   - `reallocate` 在内存用完时为 StrVec 分配新内存。

> std::allocator 是一个模板类，它没有一个显式的构造函数。它提供了一组用于内存分配和对象构造/销毁的函数，而不是一个构造函数。
> std::allocator 主要提供以下成员函数：
> - allocate: 用于分配一块指定大小的原始内存区域。
> - deallocate: 用于释放先前分配的内存区域。
> - construct: 用于在分配的内存上构造对象，通常是通过调用对象的构造函数。
> - destroy: 用于销毁在分配的内存上构造的对象，通常是通过调用对象的析构函数。

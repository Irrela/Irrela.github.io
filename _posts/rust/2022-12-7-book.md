...
- [15. 智能指针](#15-智能指针)
  ...
  - [15.5 RefCell 和内部可变性模式](#155-refcell-和内部可变性模式)
    - [通过 RefCell 在运行时检查借用规则](#通过-refcell-在运行时检查借用规则)
    - [内部可变性：不可变值的可变借用](#内部可变性不可变值的可变借用)
    - [内部可变性的用例：mock 对象](#内部可变性的用例mock-对象)
    - [RefCell 在运行时记录借用](#refcell-在运行时记录借用)
      - [使用风险](#使用风险)
    - [结合 Rc 和 RefCell 来拥有多个可变数据所有者](#结合-rc-和-refcell-来拥有多个可变数据所有者)
  - [15.6 引用循环与内存泄漏 -todo](#156-引用循环与内存泄漏--todo)
    - [避免引用循环：将 Rc 变为 Weak](#避免引用循环将-rc-变为-weak)
- [16. 无畏并发](#16-无畏并发)
  - [16.1 使用线程同时运行代码](#161-使用线程同时运行代码)
    - [使用 spawn 创建新线程](#使用-spawn-创建新线程)
    - [使用 join 等待所有线程结束](#使用-join-等待所有线程结束)
    - [线程与 move 闭包](#线程与-move-闭包)
    - [16.2 使用消息传递在线程间传送数据](#162-使用消息传递在线程间传送数据)
...
然而，如果 Rust 拒绝正确的程序，虽然会给开发者带来不便，但不会带来灾难。

RefCell<T> 正是用于`当你确信代码遵守借用规则，而编译器不能理解和确定的时候`。
...
- Box<T> 允许在`编译时`执行`不可变或可变`借用检查；
  Rc<T>仅允许在`编译时`执行`不可变`借用检查；
  RefCell<T> 允许在`运行时`执行`不可变或可变`借用检查。
- 因为 RefCell<T> 允许在运行时执行可变借用检查，所以我们可以在即便 RefCell<T> 自身是不可变的情况下修改其内部的值。

### 内部可变性：不可变值的可变借用
`在不可变值内部改变值`就是 `内部可变性` 模式。让我们看看何时内部可变性是有用的，并讨论这是如何成为可能的。
...
然而，特定情况下，`令一个值在其方法内部能够修改自身`，而`在其他代码中仍视为不可变`，是很有用的。值方法外部的代码就不能修改其值了。
...
> 使用 usize 是为了保证你的代码在不同计算机架构上都能正确工作，因为它会自动适应底层硬件的位数。这在`处理内存地址、索引数组`等场景中特别有用，因为它能够确保你的代码在不同环境中都具有一致的行为。

> #[cfg(test)] 是一个属性（attribute），用于条件编译，通常用于测试代码的标记
> 这意味着这些测试代码只有在运行测试时才会编译和执行。在发布版本中，这些测试代码将被排除，不会影响最终的可执行文件。


```rs
    impl Messenger for MockMessenger {
        // 此处的self是不可变引用， 编译无法通过因为试图修改修改 MockMessenger 来记录消息
...
- RefCell<T> 在任何时候`只允许有多个不可变借用或一个可变借用。`
...
Rc<T> 允许`对相同数据有多个所有者`，不过只能提供数据的不可变访问。
...
调用 thread::spawn 函数并传递一个闭包, 并在其中包含希望在新线程运行的代码:
```rs
    thread::spawn(|| {
        for i in 1..10 {
...
```rs
    // 通过将返回值储存在变量中来修复新建线程部分没有执行或者完全没有执行的问题。
    ...
    // 如果在这里调用join，则会先执行全部的spawned thread， 而不是与main thread交错
    // handle.join().unwrap();
...
### 线程与 move 闭包
`move` 闭包 常与 thread::spawn 一起使用，因为它允许我们`在一个线程中使用另一个线程的数据`。

可以在参数列表前使用 `move` 关键字`强制闭包获取其使用的环境值的所有权`。这个技巧在创建新线程将值的所有权从一个线程移动到另一个线程时最为实用。
...
fn main() {
    ...
    let handle = thread::spawn(|| {
        // 这里企图使用main thread的变量，但不合法
        println!("Here's a vector: {:?}", v); 
    });

    // 新建线程内部有一个 v 的引用，不过主线程立刻就使用 drop 丢弃了 v。
    // 接着当新建线程开始执行，v 已不再有效，所以其引用也是无效的。噢，这太糟了！
    drop(v); // oh no!
...
fn main() {
    ...
    // 强制闭包获取 v 的所有权
    // Note: 主线程理论上可能使新建线程的引用无效
    // 通过告诉 Rust 将 v 的所有权移动到新建线程, 这意味着我们向 Rust 保证主线程不会再使用 v
...
发送者位于上游位置，在这里可以将橡皮鸭放入河中，接收者则位于下游，橡皮鸭最终会漂流至此。代码中的一部分调用发送者的方法以及希望发送的数据，另一部分则检查接收端收到的消息。

`当发送者或接收者任一被丢弃时`可以认为通道被 `关闭（closed）`了。


我们将开发一个程序，它会在一个线程生成值向通道发送，而在另一个线程会接收值并打印出来。
...
通道的接收端有两个有用的方法：
- recv : 这个方法会阻塞主线程执行直到从通道中接收一个值。当通道发送端关闭，recv 会返回一个错误表明不会再有新的值到来了。
- 和 try_recv: 不会阻塞，相反它立刻返回一个 Result<T, E>：Ok 值包含可用的信息，而 Err 值代表此时没有任何消息。

> 如果线程在等待消息过程中还有其他工作时使用 `try_recv` 很有用：可以编写一个循环来频繁调用 try_recv，在有可用消息时进行处理，其余时候则处理一会其他工作直到再次检查。

### 通道与所有权转移
尝试在通过 tx.send 发送 val 到通道中之后将其打印出来，这会报错。
...
```rs
// 通过克隆发送者来创建多个生产者
fn send_with_multi_producer_cloned() {
    let (tx, rx) = mpsc::channel();

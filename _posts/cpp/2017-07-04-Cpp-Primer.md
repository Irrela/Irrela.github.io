...
如果 pop() 函数要返回被弹出的元素，那么它需要在内部执行一次拷贝构造操作。**这是因为在 C++ 中，函数的返回值和局部变量一样，具有自动存储期（automatic storage duration）。**当函数返回时，函数内部的局部变量会被销毁，其内存将被释放。所以在返回一个元素的拷贝之前，需要在内部创建一个该元素的拷贝，以保证返回的值是有效的。
...
int main() {
    std::vector<Person> people = {{"A", 25}, {"Bob", 30}, {"Charlie", 20}};

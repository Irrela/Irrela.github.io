...
### 缓存穿透防止机制
使用缓存穿透防止机制，如使用空对象缓存、缓存空结果等，避免大量无效的请求穿透缓存直接到达数据源。
- 可以使用布隆过滤器等机制防止缓存穿透

### 雪崩处理处理
引入随机性的缓存失效时间，或使用二级缓存，以防止大量缓存在同一时刻失效导致的雪崩效应。
...
策略： 缓存中最先进入的数据最先被淘汰。
场景： 适用于对访问时间较早的数据进行优先保留的情况。不适用于缓存中有热点数据且访问频率不均匀的情况。

***LRU (Least Recently Used):***
策略： 缓存中最近最少使用的数据最先被淘汰。
场景： 适用于访问热点数据的场景，能够有效保留最常用的数据。
***不适用于数据访问模式呈现周期性变化的情况。***

***LFU (Least Frequently Used):***
策略： 缓存中使用频率最低的数据最先被淘汰。
场景： 适用于数据访问频率相对稳定的场景，能够更精确地保留使用频率较高的数据。
***不适用于热点数据频繁变化的情况。***

***Random Replacement:***
策略： 随机选择一个数据进行淘汰。
场景： 适用于对淘汰数据没有明确优先级要求的场景。不适用于需要根据数据的访问模式进行优化的场景。

***Write-Through:***
策略： 在数据被写入时，同时将数据写入缓存和持久存储。
场景： 适用于 ***对写入操作要求实时性*** 的场景，***确保数据一致性***。不适用于读操作频繁、写操作较少的场景。

***Write-Behind (Write-Back):***
策略： 在数据被写入时，先写入缓存，然后异步地写入持久存储。
场景： 适用于对写入操作要求低延迟的场景，***可以提高写入性能***。不适用于对数据一致性要求非常高的场景。

#### 最近最常使用（LFU）缓存策略
基于数据的使用频率来淘汰最不常用的数据。如果你的应用有明显的热点数据，LFU 策略可能更合适。

#### 考虑使用写入时复制策略
对于 ***写入频率较高的缓存***，可以考虑使用 ***写入时复制（Copy-On-Write）*** 策略。
即在写操作时，将缓存项复制一份，保留原始数据，以提高读操作的效率。

#### 使用自适应缓存策略
一些缓存库提供了自适应策略，根据缓存的使用情况和访问模式自动调整缓存的大小和淘汰策略。

#### 分区缓存
将缓存划分为多个分区，每个分区可以有不同的过期时间或淘汰策略。
这样可以更 ***灵活*** 地管理不同类型的数据。

### 使用适当的过期策略
根据数据的特性和业务需求，设置合理的缓存过期时间。

> 过期时间太短可能导致频繁的缓存失效，而过期时间太长可能导致缓存中的数据过期而不再准确。
...
例如，对于热点数据，使用本地缓存（如使用 Guava Cache 或 Caffeine 等）可以提高命中率，因为本地缓存的读写速度通常比分布式缓存更快。

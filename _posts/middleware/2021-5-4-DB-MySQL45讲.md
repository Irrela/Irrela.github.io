...
- [2、日志系统：一条SQL查询语句是如何执行的？](#2日志系统一条sql查询语句是如何执行的)
  - [日志模块：redo log](#日志模块redo-log)
  - [日志模块：binlog](#日志模块binlog)
...
- [13 | 为什么表数据删掉一半，表文件大小不变？](#13--为什么表数据删掉一半表文件大小不变)
  - [参数 innodb\_file\_per\_table](#参数-innodb_file_per_table)
  - [数据删除流程](#数据删除流程)
  - [如何去除空洞，收缩表空间：重建表](#如何去除空洞收缩表空间重建表)
...
- [22 | MySQL有哪些“饮鸩止渴”提高性能的方法？](#22--mysql有哪些饮鸩止渴提高性能的方法)
  ...
  - [慢查询性能问题](#慢查询性能问题)
    - [第一种可能：索引没有设计好](#第一种可能索引没有设计好)
    - [第二种可能：语句没写好](#第二种可能语句没写好)
    - [第三种可能：MySql选错了索引](#第三种可能mysql选错了索引)
  - [QPS突增问题](#qps突增问题)
- [23 | MySQL是怎么保证数据不丢的？](#23--mysql是怎么保证数据不丢的)
  - [redo log 的写入机制](#redo-log-的写入机制)
- [24 | MySQL是怎么保证主备一致的？](#24--mysql是怎么保证主备一致的)
  - [binlog 的三种格式对比](#binlog-的三种格式对比)
  - [为什么会有 mixed 格式的binlog？](#为什么会有-mixed-格式的binlog)
  - [循环复制问题](#循环复制问题)
...
- [26 | 备库为什么会延迟好几个小时？](#26--备库为什么会延迟好几个小时)
  - [MySQL 5.5 版本的并行复制策略](#mysql-55-版本的并行复制策略)
    - [按表分发策略](#按表分发策略)
    - [按行分发策略](#按行分发策略)
...
- [29 | 如何判断一个数据库是不是出问题了？](#29--如何判断一个数据库是不是出问题了)
  - [select 1 判断](#select-1-判断)
  - [查表判断](#查表判断)
  - [更新判断](#更新判断)
  - [内部统计](#内部统计)
  - [小结](#小结-2)
- [30 | 答疑文章（二）：](#30--答疑文章二)
  - [不等号条件里的等值查询](#不等号条件里的等值查询)
  - [等值查询的过程](#等值查询的过程)
  - [怎么看死锁](#怎么看死锁)
  - [怎么看锁等待](#怎么看锁等待)
...
- [38 | 都说InnoDB好，那还要不要使用Memory引擎？](#38--都说innodb好那还要不要使用memory引擎)
  - [内存表的数据组织结构](#内存表的数据组织结构)
  - [hash 索引和 B-Tree 索引](#hash-索引和-b-tree-索引)
  - [内存表的锁](#内存表的锁)
...
- [40 | insert语句的锁为什么这么多？](#40--insert语句的锁为什么这么多)
  - [insert ... select 语句](#insert--select-语句)
  - [insert 循环写入](#insert-循环写入)
...
- [42 | grant之后要跟着flush privileges吗？](#42--grant之后要跟着flush-privileges吗)
  ...
  - [db 权限](#db-权限)
  - [表权限和列权限](#表权限和列权限)
  - [flush privilege 使用场景](#flush-privilege-使用场景)
  - [小结](#小结-10)
...
- [44 | 答疑文章（三）：说一说这些好问题](#44--答疑文章三说一说这些好问题)
  ...
  - [表定义自增值 id](#表定义自增值-id)
  - [InnoDB 系统自增 row\_id](#innodb-系统自增-row_id)
  - [Xid](#xid)
  - [InnoDB trx\_id](#innodb-trx_id)
